    ...
    async def kopfoderzahl(self, ctx):
        if ctx.channel.name == "casino":
            ...
            try:
                ...
                mycursor = mydb.cursor()
                sql1 = f"UPDATE CASINO SET COINS = COINS {lose_coins} WHERE USER_ID = '{ctx.message.author.id}'"
                mycursor.execute(sql1)
                ...
                if message.content.lower() == "kopf" or "zahl":
                    ...
                    if message.content.lower() == ergebnis:
                        ...
                        mycursor = mydb.cursor()
                        sql1 = f"UPDATE CASINO SET COINS = COINS {win_coins} WHERE USER_ID = '{ctx.message.author.id}'"
                        mycursor.execute(sql1)
                        sql3 = f"UPDATE CASINO SET WINS = WINS +1 WHERE USER_ID = '{ctx.message.author.id}'"
                        ...
                        sql2 = f"SELECT COINS FROM CASINO WHERE USER_ID = '{ctx.message.author.id}'"
                        mycursor.execute(sql2)
                        sql3 = f"UPDATE CASINO SET LOSE = LOSE +1 WHERE USER_ID = '{ctx.message.author.id}'"
                        mycursor.execute(sql3)
                        wallet = float(mycursor.fetchone()[0])
                        await ctx.send(f"Your wallet now contains {wallet} coins")
                        mydb.commit()
